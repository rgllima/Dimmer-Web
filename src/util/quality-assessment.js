...
function predictByDecisionTree(x, root, path = []) {
  const { label, rule, left, right } = root;
  const { feature, threshold } = rule || {};

  path.push(root);

  if (label) return { label, path };
  if (x[feature] <= threshold) return predictByDecisionTree(x, left, path);

  return predictByDecisionTree(x, right, path);
}
...
function sanitizeSuggestionsForImprovement(x, suggestions) {
  ...
  return deduplicatedSuggestions.filter((suggestion) =>
    filterAcceptedSuggestions(x, suggestion)
  );
}

function getImprovementSuggestions(x, prediction) {
...
export { predictByDecisionTree, getImprovementSuggestions };
